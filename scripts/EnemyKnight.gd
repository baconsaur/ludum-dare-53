...
export var max_move_interval = 0.5
...
func _process(delta):
	...
	if opponent:
		...
		if opponent.sword_position == SwordPositions.DOWN and sword_position == SwordPositions.DOWN:
			...
			action_weights[Actions.BLOCK] = 20 if strip_status["legs"] else 5
		if opponent.sword_position == SwordPositions.UP and sword_position == SwordPositions.UP:
			...
			action_weights[Actions.BLOCK] = 20 if strip_status["torso"] else 5
